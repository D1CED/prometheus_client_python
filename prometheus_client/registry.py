...
class CollectorRegistry(Collector):
    ...
    def get_sample_value(self, name: str, labels: Optional[Dict[str, str]] = None) -> Optional[Union[float, NativeHistogram]]:
        ...
        for metric in self.collect():
            for s in metric.samples:
                if s.name == name and s.labels == labels:
                    return s.value if s.value is not None else s.native_histogram
        return None
